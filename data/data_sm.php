<?php
    ...
    if($_GET['act']== 'tambah'){
        ...
        if($_FILES['filesm']['name']==''){
            mysqli_query($conn, "INSERT INTO tb_suratmasuk VALUES (NULL, '$no', '$noag', '$asal', '$kota', '$jenis', '$kla', '$isi', '$tglsm', '$tgld', 'TIDAK ADA FILE', '$ket')");
            ...
            if(!in_array($ext,$ekstensi) ) {
                ...
                if($ukuran < 5044070){ //5Mb		
                    ...
                    mysqli_query($conn, "INSERT INTO tb_suratmasuk VALUES (NULL, '$no', '$noag', '$asal', '$kota', '$jenis', '$kla', '$isi', '$tglsm', '$tgld', '$xx', '$ket')");
                    echo "<script language='javascript'>alert('Berhasil Menambah data'); document.location.href='../suratmasuk.php';</script>";
                }else{
    ...
    elseif($_GET['act']=='edit'){
        ...
        if($_FILES['filesm']['name']==''){
            mysqli_query($conn, "UPDATE tb_suratmasuk SET no_suratmasuk='$no', no_agenda='$noag', asal_surat='$asal', kota_surat='$kota', jenis_surat='$jenis', kode_klasifikasi='$kla', isi_surat='$isi', tgl_surat='$tglsm', tgl_diterima='$tgld', ket_suratmasuk='$ket', file_suratmasuk='$filelama' WHERE id_suratmasuk='$id'");
            ...
            if(!in_array($ext,$ekstensi) ) {
                ...
                if($ukuran < 5044070){ //5Mb	
                    ...
                    $xx = $rand.'_'.$filename;
                    move_uploaded_file($_FILES['filesm']['tmp_name'], '../file/suratmasuk/'.$rand.'_'.$filename);
                    mysqli_query($conn, "UPDATE tb_suratmasuk SET no_suratmasuk='$no', no_agenda='$noag', asal_surat='$asal', kota_surat='$kota', jenis_surat='$jenis', kode_klasifikasi='$kla', isi_surat='$isi', tgl_surat='$tglsm', tgl_diterima='$tgld', ket_suratmasuk='$ket', file_suratmasuk='$xx' WHERE id_suratmasuk='$id'");
                    echo "<script language='javascript'>alert('Berhasil Mengedit data'); document.location.href='../suratmasuk.php';</script>";
                    } else {
                    unlink("../file/suratmasuk/$b[file_suratmasuk]");
                    $xx = $rand.'_'.$filename;
